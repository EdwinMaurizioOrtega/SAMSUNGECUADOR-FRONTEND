...
import FormWhatsappModal from "~/components/features/modals/form-whatsapp-modal";
import FormWhatsAppModal from "~/components/features/modals/form-whatsapp-modal";
                                    ...
                                    <div className="product-variations">
                                        {
                                            colors.map(item =>
                                                <ALink href="#"
                                                       className={`${item.value.startsWith('#') ? 'color' : 'size'} ${curColor === item.name ? 'active' : ''} ${isDisabled(item.name, curSize) ? 'disabled' : ''}`}
                                                       key={"color-" + item.name}
                                                       style={{backgroundColor: `${item.value}`}}
                                                       onClick={(e) => toggleColorHandler(item)}>{item.value.startsWith('#') ? '' : item.name}</ALink>)
                                        }
                                    </div>
                                </div> : ''
                        }
                                        ...
                                        <div className="product-variations">
                                            {
                                                sizes.map(item =>
                                                    <ALink href="#"
                                                           ...
                                                           key={"size-" + item.name}
                                                           onClick={(e) => toggleSizeHandler(item)}>{item.value}</ALink>)
                                            }
                                        </div>
                                        ...
                                        <Collapse in={'null' !== curColor || 'null' !== curSize}>
                                            <div className="card-wrapper overflow-hidden reset-value-button w-100 mb-0">
                                                <ALink href='#' className='product-variation-clean'
                                                       onClick={resetValueHandler}>Clean All</ALink>
                                            </div>
                                        </Collapse>
                                    </div>
                                </div> : ''
                                                    ...
                                                    product.data.variants[curIndex].price ?
                                                        product.data.variants[curIndex].sale_price ?
                                                            <div className="product-price mb-0">
                                                                <ins
                                                                    className="new-price">${toDecimal(product.data.variants[curIndex].sale_price)}</ins>
                                                                <del
                                                                    className="old-price">${toDecimal(product.data.variants[curIndex].price)}</del>
                                                            </div>
                                                            : <div className="product-price mb-0">
                                                                <ins
                                                                    className="new-price">${toDecimal(product.data.variants[curIndex].price)}</ins>
                                                            </div>
                                                        : ""
                                                }
                                            </div> : ''
            ...
            <div className="product-form product-qty pb-0">
                ...
                <div className="product-form-group">
                    ...
                    <div className="icon-box icon-box-side">
                        ...
                        {/*<button onClick={handleClick} className={`btn-product btn-cart text-normal ls-normal font-weight-semi-bold `}><i*/}
