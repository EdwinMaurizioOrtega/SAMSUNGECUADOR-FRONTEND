                            ...
                            product.data.variants[ 0 ].color ?
                                <div className='product-form product-color'>
                                    ...
                                    <div className="product-variations">
                                        {
                                            colors.map( item =>
                                                <ALink href="#" className={ `${ item.value.startsWith( '#' ) ? 'color' : 'size' } ${ curColor === item.name ? 'active' : '' } ${ isDisabled( item.name, curSize ) ? 'disabled' : '' }` } key={ "color-" + item.name } style={ { backgroundColor: `${ item.value }` } } onClick={ ( e ) => toggleColorHandler( item ) }>{ item.value.startsWith( '#' ) ? '' : item.name }</ALink> )
                                        }
                                    </div>
                                </div> : ''
                        }
